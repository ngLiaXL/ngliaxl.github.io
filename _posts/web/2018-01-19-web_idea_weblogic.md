---
layout: post
title:  "使用Intellij IDEA编译Weblogice项目"
date:   2018-01-19 11:53:27 +0800
categories: web
---


### 下载 [Intellij IDEA](https://www.jetbrains.com/idea/)

![](/assets/web_img01.png)

注册适用下图的版本[参考](https://blog.csdn.net/gnail_oug/article/details/53977118)，首次登录点Evaluate for free 确认后Help-->Register弹窗Activation code输入如下注册码


```
CNEKJPQZEX---
```

如出现错误打开C:\Windows\System32\drivers\etc\hosts文件，添加一行

![](/assets/web_img02.png)

### 导入eclipse Weblogic项目[参考](https://www.jianshu.com/p/455c7c11dfb2) 
File-->New--Project from existion Sources选择项目所在目录
![](/assets/web_img03.png)

选择Eclipse，直到Finish

![](/assets/web_img04.png)

导入成功后编译代码，Build-->Build Project 如遇到编译错误提示

```
Error:(51, 65) java: 无法访问weblogic.descriptor.BeanUpdateListener
  找不到weblogic.descriptor.BeanUpdateListener的类文件
Error:(257, 26) java: 无法访问weblogic.utils.wrapper.WrapperImpl
  找不到weblogic.utils.wrapper.WrapperImpl的类文件

```
[参考](http://blog.csdn.net/zhyh1986/article/details/46469173)


![](/assets/web_img05.png)

### 部署项目

Edit Configurations-->添加Weblogic Server Local-->输入weblogic用户名，密码，Domain path等

![](/assets/web_img06.png)


![](/assets/web_img07.png)
Run








